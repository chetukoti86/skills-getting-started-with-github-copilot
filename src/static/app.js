      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = details.participants.length
          ? details.participants
              .map(
                (p) =>
                  `<li><span class="participant-name">${p}</span><button class="remove-participant" data-activity="${encodeURIComponent(
                    name
        ...
        activityCard.addEventListener("click", async (event) => {
          ...
          const activityName = decodeURIComponent(event.target.dataset.activity);
          const participantEmail = decodeURIComponent(event.target.dataset.email);
          ...
          try {
            ...
            messageDiv.textContent = "Unable to remove participant. Try again.";
            messageDiv.className = "error";
            messageDiv.classList.remove("hidden");
